...
class GomokuEnv(gym.Env):
    ...
    已修正圖片路徑問題，確保一定讀得到 'sprites/floor.png'。
    """
    # 設定渲染模式與 FPS
    ...
    def __init__(self, board_size=9, win_streak=5, render_mode=None):
        ...
        self.window = None
        self.clock = None
        self.background_img = None # 用來暫存背景圖片
    ...
    def _render_frame(self):
        ...
        if self.background_img is None:
            try:
                # 1. 取得 oop_project_env.py 這個檔案所在的「絕對資料夾路徑」
                # 這樣無論你在哪裡執行 python 指令，基準點都是這個檔案的位置
                current_dir = os.path.dirname(os.path.abspath(__file__))
                
                # 2. 組合路徑: sprites/floor.png
                img_path = os.path.join(current_dir, "sprites", "floor.png")
                
                # 3. 檢查檔案是否存在
                if not os.path.exists(img_path):
                    # 如果 sprites 裡面沒有，試試看是否在同一層
                    img_path = os.path.join(current_dir, "floor.png")
                
                if os.path.exists(img_path):
                    print(f"成功載入背景圖片: {img_path}")
                    loaded_img = pygame.image.load(img_path)
                    # 縮放圖片以適應視窗大小
                    self.background_img = pygame.transform.scale(loaded_img, (self.window_size, self.window_size))
                else:
                    print(f"警告：找不到圖片檔案，使用預設顏色。搜尋路徑: {img_path}")
                    self.background_img = "DEFAULT_COLOR"
        ...
        # --- 繪製背景 ---
        if self.background_img != "DEFAULT_COLOR" and self.background_img is not None:
            canvas.blit(self.background_img, (0, 0))
        else:
            canvas.fill((221, 187, 136)) # 預設背景色 (如果圖片讀取失敗)
